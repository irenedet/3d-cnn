""" Functions taken from Inferno
https://github.com/inferno-pytorch/inferno/blob/master/inferno/utils/python_utils.py
"""
...
def is_maybe_list_of(check_function):
    def decorated_function(object_, **kwargs):
        if isinstance(object_, (list, tuple)):
            return all(
                [check_function(_object, **kwargs) for _object in object_])
        else:
