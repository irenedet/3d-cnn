...
#SBATCH --mem 128G
#SBATCH --time 0-2:50
#SBATCH -o slurm_outputs/clustering_evaluation.slurm.%N.%j.out
#SBAtCH -e slurm_outputs/clustering_evaluation.slurm.%N.%j.err
...
python3 ./runners/dataset_tables/cluster_particle_picking/2_subtomograms_segmentation.py -model $path_to_model -label $label_name -dataset_table $dataset_table -tomo_name $tomo_name -init_feat $init_feat -depth $depth -out_classes $output_classes -new_loader $new_loader -BN $BN
echo '... done.'

echo 'running python3 scripts: getting particles motive list'
python3 ./runners/dataset_tables/cluster_particle_picking/3_get_cluster_centroids_motl.py -dataset_table $dataset_table -min_cluster_size $min_cluster_size -max_cluster_size $max_cluster_size -tomo_name $tomo_name -output $output_dir -label $label_name -box $box_side -class_number $class_number -particle_radius $minimum_peak_distance -overlap $box_overlap
echo '... done.'
